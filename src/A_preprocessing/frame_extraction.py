    ...
    *,
    ...
    try:
        ...
        def time_mode_iterator() -> Iterator[FrameInfo]:
            ...
            grabs_total = 0
            retrieves_total = 0
            seeks_total = 1 if t_start > 0 else 0
            ...
            try:
                while True:
                    ...
                    if progress_callback and frame_count > 0:
                        percent = int((current_idx / frame_count) * 100)
                        if percent > last_progress:
                            progress_callback(percent)
                            last_progress = percent

                    ts_ms = float(cap.get(cv2.CAP_PROP_POS_MSEC) or 0.0)
                    effective_sec = _effective_timestamp(ts_ms, current_idx)
                    ...
                    if bad_ts_streak >= max_bad_ts:
                        ...
                        yield from index_mode_iterator(
                            fallback_stride=stride, start_idx_offset=read_idx
                        )
                        return
                    ...
                    if ts_ms > 0.0 and ts_ms + SEEK_SLACK_MS < next_target_ms:
                        ...
                        if ts_ms > 0.0 and ts_ms + SEEK_SLACK_MS < next_target_ms:
                            if behind_counter % _SEEK_RETRY_INTERVAL == 0:
                                ...
                                seeks_total += 1
                                if fps_base > 0:
                                    approx_idx = int(round((next_target_ms / 1000.0) * fps_base))
                                    read_idx = max(read_idx, approx_idx)
                            continue
                    else:
                        behind_counter = 0
                    ...
                    yield FrameInfo(
                        index=current_idx,
                        timestamp_sec=_effective_timestamp(ts_ms, current_idx),
                        array=frame,
                        width=w,
                        height=h,
                    )
