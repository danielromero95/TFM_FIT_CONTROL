    ...
    *,
    ...
    for col in selected:
        if col in df.columns:
            column_series = _series_list(df[col].tolist())
            series[col] = column_series[::stride]
...
def render_video_with_metrics_sync(
    *,
    ...
    """
    Renderiza un visor combinado: vídeo + gráfica Plotly con cursor sincronizado.
    - El cursor vertical sigue el currentTime del vídeo.
    - Click en la gráfica -> seek del vídeo al instante X.
    """
    try:
...
(function() {
  ...
  ["timeupdate","seeked"].forEach((ev) => {
    video.addEventListener(ev, updateCursorFromVideo);
  });
  video.addEventListener("loadedmetadata", () => {
    if (Number.isFinite(DATA.startAt)) {
      try {
        video.currentTime = Math.max(0, DATA.startAt);
      } catch (err) {
        console.warn("StartAt set error:", err);
      }
    }
