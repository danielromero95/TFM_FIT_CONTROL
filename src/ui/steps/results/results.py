    ...
    with step_container("results"):
        ...
        if state.pipeline_error:
            ...
            if not rep_speeds_df.empty:
                metric_lower = (primary_metric or "").lower()
                if "deg" in metric_lower or "angle" in metric_lower:
                    speed_unit = "deg/s"
                elif metric_lower.endswith("_m") or "height" in metric_lower:
                    speed_unit = "m/s"
                else:
                    speed_unit = "units/s"
                ...
                rep_chart_df = rep_speeds_df.rename(
                    columns={
                        "Down speed (units/s)": down_speed_col,
                        "Up speed (units/s)": up_speed_col,
                    }
                )
                rep_chart_df = rep_chart_df.melt(
                    id_vars=[
                        "Repetition",
                        "Down duration (s)",
                        "Up duration (s)",
                ...
                rep_chart_df["Phase duration (s)"] = rep_chart_df.apply(
                    lambda row: row["Down duration (s)"]
                    if "Down" in str(row["Phase"])
                    else row["Up duration (s)"],
                    axis=1,
                )
