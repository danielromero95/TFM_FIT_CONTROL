...
def _configure_step(*, disabled: bool = False, show_actions: bool = True) -> None:
    with step_container("configure"):
        ...
        candidate_labels = [metrics_catalog.human_metric_name(metric) for metric in candidates]
        if candidate_labels:
            candidate_list = " or ".join(candidate_labels)
            st.caption(
                f"The app will automatically select the most reliable rep-counting angle from "
                f"{candidate_list}, considering visibility, tracking quality, and availability."
            )
        else:
            st.caption(
                "The app will automatically select the most reliable rep-counting angle based "
                "on metric visibility, tracking quality, and availability."
            )
