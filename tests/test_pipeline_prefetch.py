...
from src.exercise_detection.types import make_detection_result
...
def test_run_pipeline_uses_prefetched_detection(monkeypatch, tmp_path) -> None:
    ...
    prefetched_detection = make_detection_result(
        ...
        0.9,
        side="left",
        view_stats={"shoulder_width": np.float32(0.5)},
        debug={
            "features_summary": {"torso_tilt_deg": {"min": np.float64(1.0)}},
            "classification_scores": {"deadlift_veto": np.bool_(False)},
            "rep_count": np.int64(3),
        },
    )
    ...
    detection_block = report.debug_summary["detection"]
    assert detection_block["source"] == "prefetched"
    assert detection_block["exercise_detection_debug"] is not None
    assert detection_block["exercise_detection_debug"]["rep_count"] == 3
    assert detection_block["exercise_detection_debug"]["classification_scores"]["deadlift_veto"] is False
    assert detection_block["view_stats"]["shoulder_width"] == 0.5
    assert report.debug_summary["selection"]["user_confirmed_detection"] is True


def test_run_pipeline_normalizes_empty_selected_view(monkeypatch, tmp_path) -> None:
    video_path = tmp_path / "video.mp4"
    video_path.write_bytes(b"fake video content")
    ...
    monkeypatch.setattr(
        "src.C_analysis.pipeline.compute_metrics_and_angle",
        lambda *_args, **_kwargs: (
            pd.DataFrame({cfg.counting.primary_angle: [90.0, 80.0]}),
            25.0,
            [],
            None,
            cfg.counting.primary_angle,
    ...
    monkeypatch.setattr(
        "src.C_analysis.pipeline.maybe_count_reps",
        lambda *_args, **kwargs: (1, [], None) if kwargs.get("return_debug") else (1, []),
    )
    ...
    prefetched_detection = make_detection_result(
        ...
        "front",
        0.9,
        debug={"classification_scores": {}, "rep_count": 1},
    )
    ...
    assert report.debug_summary["exercise"]["selected_view"] is None
    assert report.debug_summary["selection"]["selected_view"] is None
