...
def test_run_pipeline_exports_detection_diagnostics(monkeypatch, tmp_path) -> None:
    video_path = tmp_path / "video.mp4"
    video_path.write_bytes(b"fake video content")
    ...
    diagnostics = {"arm_proxy_source": "elbow"}
    ...
    assert report.stats.detection_diagnostics is not None
    assert report.stats.detection_diagnostics.get("arm_proxy_source") == "elbow"
