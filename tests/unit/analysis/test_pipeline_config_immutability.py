...
def test_run_pipeline_does_not_mutate_input_config(monkeypatch, tmp_path) -> None:
    ...
    base_prominence = cfg.counting.min_prominence
    base_distance = cfg.counting.min_distance_sec
    base_refractory = cfg.counting.refractory_sec
    ...
    assert cfg.counting.min_prominence == base_prominence
    assert cfg.counting.min_distance_sec == base_distance
    assert cfg.counting.refractory_sec == base_refractory

    assert report_one.config_used.counting.min_prominence == base_prominence + 1.0
    assert report_one.config_used.counting.min_distance_sec == base_distance + 0.1
    assert report_one.config_used.counting.refractory_sec == base_refractory + 0.05

    assert report_two.config_used.counting.min_prominence == base_prominence + 1.0
    assert report_two.config_used.counting.min_distance_sec == base_distance + 0.1
    assert report_two.config_used.counting.refractory_sec == base_refractory + 0.05

    assert report_one.config_used.to_serializable_dict() == report_two.config_used.to_serializable_dict()
